...
export class GeneticPrompt {
  ...
  static async evolve(originalPrompt: string, goal: string): Promise<string> {
    ...
    for (let gen = 0; gen < GENERATIONS; gen++) {
      ...
      const scored = await Promise.all(variations.map(async (p) => ({
          prompt: p,
  ...
  private static async generateVariations(prompt: string, goal: string): Promise<string[]> {
      const response = await pollinations.chat([
          { role: 'system', content: 'You are an Evolutionary Prompt Engineer. Generate 3 distinct mutations of the user prompt that might achieve the goal better. Return strictly valid JSON array of strings.' },
  ...
  private static async scorePrompt(prompt: string, goal: string): Promise<number> {
      const response = await pollinations.chat([
          { role: 'system', content: 'Score this prompt from 0-100 on its likelihood to achieve the goal. Return strictly valid JSON: { "score": number }.' },
          { role: 'user', content: `Prompt: ${prompt}\nGoal: ${goal}` }
      ], { model: 'openai', jsonMode: true });
